...
class infer(_Jac.Walker):
    ...
    def init_router(self, _jac_here_: _Jac.RootType) -> None:
        if _Jac.visit_node(self, (lambda x: [i for i in x if isinstance(i, Router)])(_Jac.edge_ref(_jac_here_, target_obj=None, dir=_Jac.EdgeDir.OUT, filter_func=None, edges_only=False))):
            ...
            _Jac.connect(left=router_node, right=ClassClownChat(), edge_spec=_Jac.build_edge(is_undirected=False, conn_type=None, conn_assign=None))
...
@_Jac.make_node(on_entry=[_Jac.DSFunc('respond', infer)], on_exit=[])
@__jac_dataclass__(eq=False)
class ClassClownChat(Chat, _Jac.Node):
    chat_type: ChatType = _Jac.has_instance_default(gen_func=lambda: ChatType.CLASS_CLOWN)

    def respond(self, _jac_here_: infer) -> None:
        jokes = ['Why dont scientists trust atoms? Because they make up everything!', 'What do you get if you cross a snowman and a vampire? Frostbite.', 'Why did the scarecrow win an award? Because he was outstanding in his field!']
        if _jac_typ.TYPE_CHECKING:
            from random import choice
        else:
            choice, = __jac_import__(target='random', base_path=__file__, lng='py', absorb=False, mdl_alias=None, items={'choice': None})
        joke = choice(jokes)
        _jac_here_.response = joke
